% Uses DFBAlab to run a dynamic FBA simulation of a P. tricornutum and P.
% haloplanktis co-culture, simulating different DOM releas fractions
                ...
                BacterialConcentration = .01;
                global DiatomConcentration;
                DiatomConcentration = .01;
   ...
   BIOMASS_fractions = [ .1 .2 .3 .4 .5 .6 .7 .8 .9 ];
   DOM_fractions = [.9 .8 .7 .6 .5 .4 .3 .2 .1 ];

   for bf = 1:length(BIOMASS_fractions)
                 
  dia_DFBA = removeRxns(dia_DFBA,'DOM_reaction');
  dia_DFBA = addReaction(dia_DFBA,'DOM_reaction', Metabolites_Exu_reaction,[-1 BIOMASS_fractions(bf) DOM_fractions(bf)], 'false', 0, 100) ;
    ...
    P_uptake_dia = [];    
               ...
               for k=1:length(InfluxP)
                ...
                %% P. haloplanktis lexicographic optimization parameters
                ...
                other_aa_initial_conc = repelem(.001', 19);   
        ...
        %         fprintf('P uptake bacterium%d. \n',tint);
        %         P_tac
        % 
        %           P_uptake_tac{end+1, b} = P_tac;
        %           P_uptake_dia{end+1, b} = P_tac;
    ...
    % Y6 = Photons (mmol/L)
    % Y7 = Glutamate through DOM reaction
    % Y8 = Threonine through DOM reaction
    % Y9 = Asparagine through DOM reaction
    % Y10 = Serine through DOM reaction
    % Y11 = Aspartate through DOM reaction
...
%% Plots
         ...
         hold on
         plot(T/24, log10(Y(:,3)./Y(:,2)), '-','LineWidth',1.5);xlabel('Time (days)');ylabel('\fontsize{12}log_1_0 D/B biomass ratio');        
